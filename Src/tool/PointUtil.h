...
private:
    ...
    const cv::Mat StructuringElement3 = getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(3, 3));
    const cv::Mat StructuringElement5 = getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(5, 5));
    const cv::Mat StructuringElement7 = getStructuringElement(cv::MORPH_ELLIPSE, cv::Size(7, 7));
public:
    ...
    static inline cv::RotatedRect boundingRRect(const cv::RotatedRect &rect_1, const cv::RotatedRect &rect_2) {
        ...
        float short_1 = Util::findExtremumOfSide(wh_1, SHORT_SIDE);
        float long_2 = Util::findExtremumOfSide(wh_2, LONG_SIDE);
        float short_2 = Util::findExtremumOfSide(wh_2, SHORT_SIDE);
        float width = (float) (Util::pointDistance(pl, pr) - ((double) short_1 + (double) short_2) / 2.0);
        //float height = std::max(long_2, long_1);
