...
void CCalibration::calibProcess() {
    ...
    for(int i=0; i < image_Seq.size(); i++) cv::find4QuadCornerSubpix(image_Seq[i], cornersSeq[i], cv::Size(5,5));
...
//用于摄像头标定的外部调用接口
bool CCalibration::cameraCali(bool cameraPosition, const std::string& serialNumber) {
    //标定图存储的地址
    std::string patternLeftImgPath = LEFT_PATTERN_IMG_PATH;
    FileOperation::createDirectory(patternLeftImgPath);
    std::string patternRightImgPath = RIGHT_PATTERN_IMG_PATH;
    FileOperation::createDirectory(patternRightImgPath);
    //标定相机参数文件存储的地址
    ...
    //标定相机参数文件名
    std::string caliCameraDataName = CALI_BIAS_DATA_NAME;
    //标定图像素偏差参数文件名
    std::string caliBiasDataName = serialNumber + ".yml";
    //标定棋盘图像的行列数
    cv::Size boardSize = cv::Size(11, 8);
    //摄像头标定类初始化
    if (cameraPosition) {
        CCalibration calibration(patternRightImgPath, calibResultPath, caliBiasDataName, caliCameraDataName, boardSize);
        //开始摄像头标定
        calibration.run();
    }else{
        CCalibration calibration(patternLeftImgPath, calibResultPath, caliBiasDataName, caliCameraDataName, boardSize);
        //开始摄像头标定
        calibration.run();
    }
    //畸变图像修复类初始化
